...
runner_board = [[0 for _ in range(N)] for _ in range(N)]  # runner 보드
...
def bfs(s):
    ...
    while (que):
        ...
        if (runner_board[cr][cc] != 0):
            ret.append(runner_board[cr][cc])
...
def sool_move(s, k):
    r, c = s
    global order, soolae_dir, s_moves

    if (order == 0):
        nr, nc = [r + s_dir[soolae_dir][0] * s_moves, c + s_dir[soolae_dir][1] * s_moves]
        soolae_dir = (soolae_dir + 1) % 4

        if (check_border([nr, nc])):
            if (k % 2 == 0):
                s_moves += 1
        else:  # 밖으로 나갔을 때...
            nr, nc = [0, 0]
...
for k in range(1, K + 1):
    ...
    for rid in move_runner:
        ...
        if ([nr, nc] != soolae):
            runner[rid] = [nr, nc]
            runner_board[cr][cc] = 0
            runner_board[nr][nc] = rid

    # 2. 술래 이동
    # - 달팽이 모양 이동 ([n//2, n//2] -> 위1 -> 오른1 -> 아래2 -> 왼2)
    # - 이동 방향이 틀어지는 지점이라면 술래가 바라보는 방향 바로 틀기
    soolae = sool_move(soolae, k)
    ...
    for i in range(3):
        ...
        if (not tree_board[watching[0]][watching[1]]):
            if (runner_board[watching[0]][watching[1]]):
                rid = runner_board[watching[0]][watching[1]]
                runner_alive[rid] = False
                rid_r, rid_c = runner[rid]
                runner_board[rid_r][rid_c] = 0
                answer += k
                # - answer += k
